...
__global__ void total(float *input, float *output, int len) {
  ...
  //@@ correct index
  __shared__ float segment[(BLOCK_SIZE<<1)];
  unsigned id = blockIdx.x*(BLOCK_SIZE<<1) + threadIdx.x;
  
  segment[threadIdx.x] = (id < len)? input[id]: 0.0f;
  segment[threadIdx.x+BLOCK_SIZE] = (id+BLOCK_SIZE < len)? input[id+BLOCK_SIZE]: 0.0f;
  
  for(unsigned stride = BLOCK_SIZE; stride > 0; stride /= 2) {
    __syncthreads();
